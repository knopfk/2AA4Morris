...
public class GameDisplay extends JFrame implements ActionListener {
	...
	int redToAdd = allowable;
	int blueToAdd = allowable;
	...
	private JLabel redCount = new JLabel(Integer.toString(redToAdd),JLabel.LEFT);
	private JLabel blueCount = new JLabel(Integer.toString(blueToAdd),JLabel.RIGHT);
	...
	public GameDisplay(String title){
		...
		gc2.gridx = 1;
		gc2.gridy = 0;
		piecePanel.add(text, gc2);
		gc2.gridx = 0;
		gc2.gridy = 1;
		piecePanel.add(blueCount, gc2);
		gc2.gridx = 2;
		gc2.gridy = 1;
		piecePanel.add(redCount, gc2);
		...
		buttonSave.setActionCommand("Save Game");
		JButton buttonLoad = new JButton("Load Game?");
		buttonLoad.setActionCommand("Load Game");
		...
		gc1.gridx = 0;
		gc1.gridy = 2;
		buttonPanel.add(buttonSave, gc1);
		gc1.gridx = 2;
		gc1.gridy = 2;
		buttonPanel.add(buttonLoad, gc1);
		...
		gamePanel.addMouseListener(new MouseAdapter(){
				...
				for(int i = 0; i < 2; i ++){ //for each disk on the board
					for(int j = 0; j < 8; j++){
						if(gamePanel.getShapeArray()[i][j].contains(e.getPoint()) && blueTake == true){ //if the piece was clicked and it is blue's turn
							...
							text.setText("Add a Red Piece");
							play1count ++;
							redCount.setText(Integer.toString(--blueToAdd));
							state1();
						}
							...
							text.setText("Add a Blue Piece");
							play2count ++;
							blueCount.setText(Integer.toString(--redToAdd));
							state1();
						}
					}
				}
						...
						if(gamePanel.getShapeArray()[i][j].contains(e.getPoint()) && blueTake == true){ //if the piece was clicked and it is blue's turn	
							if(gamePanel.getVisibleTeams(i,j) == 1 && prevDisk[0] == -1){
								...
								if(checkMill(i,j,1)){
									...
									text.setText("It's Red's turn to move");
								}
							}
						}
						else if(gamePanel.getShapeArray()[i][j].contains(e.getPoint()) && redTake == true){//if the piece was clicked and it is red's turn
							if(gamePanel.getVisibleTeams(i,j) == 2 && prevDisk[0] == -1){
								...
								if(checkMill(i,j,2)){
									...
									blueTake = true; // it is no longer red's turn
									redTake = false; // it is blue's turn
									text.setText("It's Blue's turn to move");
			...
			if(currentState ==3){
				...
				for(int i = 0; i < 2; i ++){ //for each disk on the board
					for(int j = 0; j < 8; j++){
						if(gamePanel.getShapeArray()[i][j].contains(e.getPoint()) && blueTake == true){ //if the piece was clicked and it is blue's turn	
							text.setText("Remove a Blue Piece");
							if(gamePanel.getVisibleTeams(i,j) == 2 && prevDisk[0] == -1){
							...
							text.setText("Remove a Red Piece");
							if(gamePanel.getVisibleTeams(i,j) == 1 && prevDisk[0] == -1){
								gamePanel.setVisibleTeams(i,j,0);
								play2count--;
	...
	public void state0(){
		...
		if(random.nextBoolean() == true){
			redTake = true;
			blueTake = false;
			text.setText("Red goes first! Add a Red Piece");
		}
		else{
			blueTake = true;
			redTake = false;
			text.setText("Blue goes first! Add a Blue Piece");
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// checks if save game was pressed and saves game
		else if ("Save Game".equals(e.getActionCommand())){
			text.setText("Game Saved");
		}
		// checks if load game was pressed and loads game
		else if ("Load Game".equals(e.getActionCommand())){
			text.setText("Game Loaded");
		}
...
//	public void AddText(int display){
//		if (display == 1)
//			
//		else if (display == 2)
//			
